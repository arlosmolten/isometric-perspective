...
export class TileIsoSettings extends FormApplicationV2 {
  ...
  activateListeners(html) {
    ...
    // Use createAdjustableButton helper for offset
    ...
    html.on('click', '.select-wall', () => this._handleWallSelection(linkedWallInput));
    html.on('click', '.clear-wall', () => this._clearLinkedWalls(linkedWallInput));
  }
  ...
  const currentWallIds = Array.from(this.document.getFlag(MODULE_ID, 'linkedWallIds') ?? []);
        if (!currentWallIds.includes(selectedWallId)) {
          ...
          linkedWallInput.val(currentWallIds.join(', '));
        }
      } finally {
